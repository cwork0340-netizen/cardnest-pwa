...
export default function HeroStatusCard({ total, remaining, budget, fixedMonthlyAmount, estimatedTotal, status, statusText }) {
  ...
  return (
    <div className={`hero-card hero-card--${status}`}>
      <div className="hero-card-left">
        ...
        <div className="hero-card-amount-label">本月刷卡（含訂閱・分期）</div>
        ...
        <div className="hero-card-breakdown">
          ...
          {fixedMonthlyAmount > 0 && (
            <span className="hero-card-breakdown-item">訂閱・分期 {fmt(fixedMonthlyAmount)}</span>
          )}
        </div>

...
// 訂閱／分期到期自動材料化的交易，跟信件匯入一樣是「系統生成、還沒人為確認」，
// 對帳邏輯共用同一套：postedDate 有值才算對過帳，不然就是待對帳。
function isImported(tx) {
  return tx.autoGenerated || String(tx.note ?? '').includes('自動匯入')
}

function sourceLabel(tx) {
  if (tx.autoGenerated) return tx.category === '訂閱' ? '訂閱扣款' : '分期扣款'

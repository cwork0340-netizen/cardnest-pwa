...
function enrichPlans(plans, transactions) {
  ...
  return plans.map(p => {
    const pendingConfirmCount = transactions.filter(
      tx => tx.planId === p.id && tx.autoGenerated && !tx.confirmed
    ).length
...
export default function App() {
  ...
  // 對得起來，不管是分期還是訂閱。用同樣輸入重跑會得到空陣列，天生冪等。
  useEffect(() => {
    const { newTransactions } = materializeDuePlanTransactions({ plans, transactions, cards, today: new Date() })
  ...
  const handleMarkPaid = useCallback((id) => {
    ...
    const pendingTx = transactions
      .filter(tx => tx.planId === id && tx.autoGenerated && !tx.confirmed)
      .sort((a, b) => a.date.localeCompare(b.date))
    if (pendingTx.length > 0) {
      const targetId = pendingTx[0].id
      setTransactions(prev => prev.map(t => t.id === targetId ? { ...t, confirmed: true, postedDate: todayStr } : t))
      return
    }
    ...
    try {
      ...
      const findCandidate = (mappedCard, rawDate) => {
        ...
        return transactions.filter((tx) => {
          if (!tx.planId || !tx.autoGenerated || tx.confirmed || claimed.has(tx.id)) return false
